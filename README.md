...
Start with `AGENTS.md` for the local maintenance playbook, `PLANS.md` for repo-local ExecPlan rules, `plan.md` for the current worked plan example, `SKILL.md` for the short repo entrypoint, `docs/sysadmin-main-multi-agent-sop.md` for extended workflow notes, and `docs/windows-sandbox-validation.md` for risky manual validation plus the automated Sandbox `-WhatIf` capture flow.
