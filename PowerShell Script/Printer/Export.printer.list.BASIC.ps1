...
function Invoke-BasicPrinterExport {
    [CmdletBinding(SupportsShouldProcess = $true)]
    param(
        [Parameter(Mandatory = $true)]
    ...
    $SecureOutputDirectory = Resolve-SecureDirectory -Path $OutputDirectory -AllowedRoots @($StorageRoot)
